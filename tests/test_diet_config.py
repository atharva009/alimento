...
# ---------------------------------------------------------------------------
# TC-07  calculate_macro_grams
# ---------------------------------------------------------------------------
class TestCalculateMacroGrams:
    """
    Test case TC-07: Convert calorie target to macro grams.
 
    standard_american macro ratios: carbs=50%, protein=20%, fat=30%
    At 2000 kcal:
      carbs   = 2000 × 0.50 / 4 = 250 g
      protein = 2000 × 0.20 / 4 = 100 g
      fat     = 2000 × 0.30 / 9 ≈  66.67 g
 
    Oracle: returns dict with keys 'carbs', 'protein', 'fat' matching above.
    Success: values match within 0.1 g tolerance.
    Failure: wrong values or missing keys.
    """
 
    def test_standard_american_2000_kcal(self):
        result = calculate_macro_grams(2000, "standard_american")
        assert result["carbs"]   == pytest.approx(250.0,  abs=0.1)
        assert result["protein"] == pytest.approx(100.0,  abs=0.1)
        assert result["fat"]     == pytest.approx(66.67,  abs=0.1)
 
    def test_ketogenic_2000_kcal(self):
        # ketogenic: carbs=5%, protein=25%, fat=70%
        result = calculate_macro_grams(2000, "ketogenic")
        assert result["carbs"]   == pytest.approx(25.0,   abs=0.1)   # 2000×0.05/4
        assert result["protein"] == pytest.approx(125.0,  abs=0.1)   # 2000×0.25/4
        assert result["fat"]     == pytest.approx(155.56, abs=0.1)   # 2000×0.70/9
 
    def test_unknown_diet_falls_back_to_standard_american(self):
        """An unrecognised diet slug must fall back to standard_american ratios."""
        known = calculate_macro_grams(2000, "standard_american")
        fallback = calculate_macro_grams(2000, "nonexistent_diet_xyz")
        assert fallback["carbs"]   == pytest.approx(known["carbs"],   abs=0.1)
        assert fallback["protein"] == pytest.approx(known["protein"], abs=0.1)
        assert fallback["fat"]     == pytest.approx(known["fat"],     abs=0.1)
 
    def test_returns_dict_with_required_keys(self):
        result = calculate_macro_grams(1800, "mediterranean")
